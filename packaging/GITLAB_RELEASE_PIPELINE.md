...
| Tag | 架构 | 用途 |
|---|---|---|
| `macos-13-arm64` | arm64 | Apple Silicon 构建和最低系统验收 |
| `macos-13-x86_64` | x86_64 | Intel 构建和最低系统验收 |

Runner 应是专用真机或受控虚拟机，不能与开发者日常环境共用。构建账户不得把个人 Homebrew、Conda、Python 配置暴露给发布 Job。
...
validate
  -> test
  -> build_arm64 / build_x86_64
  -> audit_arm64 / audit_x86_64
  -> packaged_test_arm64 / packaged_test_x86_64
  -> sign
  -> notarize
  -> dmg
...
每个架构的中间 Artifact 必须带 Commit SHA、版本和架构，防止交叉使用。
